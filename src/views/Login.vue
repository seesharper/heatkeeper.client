...
<script lang="ts">
import {LoggedInUser} from "../models/models"
import Vue from "vue";
...
export default class Login extends Vue {
    ...
    async handleSubmit():Promise<void>{
      ...
      var response = await fetch(url, {method:'POST',headers : requestHeaders,body:JSON.stringify({userName : this.userName, password: this.password})})
      if (response.status == 200){
        var authResponse = await response.json();
        var loggedInUser = <LoggedInUser>authResponse;
